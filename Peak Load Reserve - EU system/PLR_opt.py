# -*- coding: utf-8 -*-
"""
Created on Tue Apr 12 15:25:38 2016
...
def DayAheadMarket():    
    ...
    priceopt = pd.read_csv('startingpoint.csv', sep=',', encoding='latin-1').set_index('time')
    windopt = pd.read_csv('startingpoint.csv', sep=',', encoding='latin-1').set_index('time')
    solaropt = pd.read_csv('startingpoint.csv', sep=',', encoding='latin-1').set_index('time')
    loadshedopt = pd.read_csv('startingpoint.csv', sep=',', encoding='latin-1').set_index('time')
    zonalconsumptionopt = pd.read_csv('startingpoint.csv', sep=',', encoding='latin-1').set_index('time') 
    ...
    WindLoad_df = pd.DataFrame({'Time':WindPenetration.index, 'WindProduction[MW]':windopt.sum(axis=1).values, 'SolarProduction[MW]':solaropt.sum(axis=1).values, 'TotalConsumption[MW]': Consumption.values, 'WindPenetration[%]':WindPenetration.values, 'SolarPenetration[%]':SolarPenetration.values}).set_index('Time')
...
def plantinvestment(df_price_DA, df_price, zones, gens_for_zones, timeperiod, df_capacityreq):
    ...
    geninv = geninv[geninv.primaryfuel != 'Oil']
    geninv = geninv[geninv.primaryfuel != 'Nuclear']
