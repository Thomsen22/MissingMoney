# -*- coding: utf-8 -*-
"""
Created on Tue Apr 26 11:36:48 2016
...
# This class finds the cheapest PLR in each zone
...
class PLRclass:
    ...
    def _load_data(self):
        ...
        self.data.plrdemand = 0.15
        self.data.flow, self.data.windpenlevel, self.data.totalwindcost = plrmodel.DayAheadMarket() # Runs DA market clearing
        ...
        # Assigning each node to a zone/country (price-area, zone)
        ...
        # Connection between zones (tells which zones are connected to each other)
        self.data.zonecons = {}
        ...
        self.data.peakloadhours = {}
        for z in zones:
            self.data.peakloadhours[z] = df_zonalconsumption[z].argsort().values[-3:]

        self.data.peaktimes = np.arange(len(self.data.peakloadhours))

        self.data.hours_for_zones = {}
        for z in zones:
            for t in self.data.peaktimes:
                self.data.hours_for_zones.setdefault(z,[]).append(self.data.peakloadhours[z][t])

        # Argument: peakload hours are presumably the same in neighbouring zones (How much power transfer between zones in peak load periods)
        self.data.peakflow = {}
        for l in self.data.lines:
            self.data.peakflow[l] = max(abs(self.data.flow[l][t]) for t in self.data.hours_for_zones[l[0]])
      
        self.data.linelimit = {}
        for l in self.data.lines:
            if self.data.lineinfo[l]['linecapacity'] - self.data.peakflow[l] > 0:
                self.data.linelimit[l] = self.data.lineinfo[l]['linecapacity'] - self.data.peakflow[l]
            elif self.data.lineinfo[l]['linecapacity'] - self.data.peakflow[l] <= 0:
                self.data.linelimit[l] = 0
    ...
    def _build_variables(self):
        ...
        for l in lines: 
            self.variables.linelimit[l] = m.addVar(lb=-linelimit[l], ub=linelimit[l])
